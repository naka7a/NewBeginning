...
<head>
    ...
    <script src="<c:url value="/resources/js/main.js" />"></script>
	    ...
	    <link rel="stylesheet" href="<c:url value="/resources/css/microtip.min.css" />"> <!-- Tooltip -->
	    <link rel="stylesheet" href="<c:url value="/resources/css/selectize.apple.css" />"> <!-- Tooltip -->
...
function getAllRestaurants() {
		...
		$.ajax({
			...
			for(var i in response)
				$("#myWrapper").append("<div class='rst rst-3'>"
					+"<div class='rst-hero'></div>"
					+"<div class='rst-info'>"
					+"<h3><a href="+ctx+"/app/restaurant-details/"+response[i].id+">"+response[i].nume+"</a></h3>"
					+"<span class='rst-description'>"+response[i].descriere+"</span>"
					+"</div></div>");
			},
			error : function() {
				alert("eroare");
